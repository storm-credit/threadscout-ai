...
test('specialist commands are supervised by Orchestrator and return control to it', async () => {
  await withServer(async (baseUrl) => {
    ...
    const candidate = added.today.candidates.find((item) => item.id === added.candidateId);
...
test('Orchestrator rejects specialist dispatch from an invalid workflow state', async () => {
  await withServer(async (baseUrl) => {
    ...
    assert.equal(result.orchestrationReceipt, null);
  });
});
